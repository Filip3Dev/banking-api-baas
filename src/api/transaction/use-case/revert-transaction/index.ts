...
export class RevertTransaction implements BaseUseCase {
  constructor(
  ...
  async handle(request: TransactionRequest): Promise<Transaction> {
    ...
    const { externalIdentifier, description, externalUserId, symbol, transactionTypeSlug, reason } = request
    ...
    const oldTransaction = await this.transactionRepository.findOneOrFail({ id: Number(externalIdentifier) })
